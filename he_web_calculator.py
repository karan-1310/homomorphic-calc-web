...
# Function to safely evaluate parsed AST nodes on encrypted numbers
def eval_expr(node, context):
    if isinstance(node, ast.Num):  # if number, encrypt and return
        return ts.ckks_vector(context, [node.n])
    
    elif isinstance(node, ast.BinOp):  # if binary operation
